...
void SysMC::smltAnnl(int& flag,const double temIni,const double temEnd,const double sclFct,const int winAdjIntv){
    flag=0;
...
Tsk::Tsk(const int numPrmIn){
    this->numPrm=numPrmIn;
    this->prmStng=new double*[this->numPrm];
    for(int i=0;i<this->numPrm;i++){
        this->prmStng[i]=new double[3];
    }
    this->numSlcList=new int[this->numPrm];
}

Tsk::Tsk(const Tsk& obj){
    this->numPrm=obj.numPrm;
    this->prmStng=new double*[this->numPrm];
    for(int i=0;i<this->numPrm;i++){
        this->prmStng[i]=new double[3];
        for(int j=0;j<3;j++){
            this->prmStng[i][j]=obj.prmStng[i][j];
        }
    }
    this->numSlcList=new int[this->numPrm];
    for(int i=0;i<this->numPrm;i++){
        this->numSlcList[i]=obj.numSlcList[i];
    }
}

Tsk& Tsk::operator=(const Tsk& obj){
    if(this==&obj){
        return *this;
    }

    for(int i=0;i<this->numPrm;i++){
        delete[] this->prmStng[i];
    }
    delete[] this->prmStng;
    delete[] this->numSlcList;
...
void Tsk::set(const vector<double>& prmMinList,const vector<double>& prmMaxList,const vector<int>& numSlcListIn){
    bool jud1=(prmMinList.size()==this->numPrm);
    bool jud2=(prmMaxList.size()==this->numPrm);
    bool jud3=(numSlcListIn.size()==this->numPrm);
    bool jud=(jud1&&jud2&&jud3);
    if(!jud){
        for(int i=0;i<this->numPrm;i++){
            this->numSlcList[i]=-1;
            for(int j=0;j<3;j++){
    ...
    for(int i=0;i<this->numPrm;i++){
        this->numSlcList[i]=numSlcListIn[i];
    }

    vector<double> prmSpcList(this->numPrm);
    for(int i=0;i<this->numPrm;i++){
        if(numSlcList[i]>1){
            prmSpcList[i]=(prmMaxList[i]-prmMinList[i])/(numSlcListIn[i]-1);
        }else{
            prmSpcList[i]=0;
        }
    }
    ...
    for(int i=0;i<this->numPrm;i++){
        this->prmStng[i][0]=prmMinList[i];
        this->prmStng[i][1]=prmMaxList[i];
        this->prmStng[i][2]=prmSpcList[i];
    }
}

void Tsk::codeToPrmSet(const int code,vector<double>& prm)const{
    vector<int> codeList(this->numPrm);
    int temp=code;
    for(int i=this->numPrm-1;i>-1;i--){
        codeList[i]=temp%this->numSlcList[i];
        temp=temp/this->numSlcList[i];
    }
    for(int i=0;i<this->numPrm;i++){
        prm[i]=this->prmStng[i][0]+this->prmStng[i][2]*codeList[i];
    }
}

int Tsk::calcNumPrmSet()const{
    int res=1;
    for(int i=0;i<this->numPrm;i++){
        res*=this->numSlcList[i];
    }
    return res;
}