...
int main(){
    ...
    if(comm_rank!=0){
        ...
        /*import codes to be verified*/
        vector<int> prmSetCodeList;
        ifstream fin;
        fin.open(inLoc+"prmSetCodeList.dat");
        int tempIn;
        while(!fin.eof()){
            fin>>tempIn;
            prmSetCodeList.push_back(tempIn);
        }
        fin.close();

        int prmSetCode=0;
        int prmSetCnt=0;
        int numPrmSet=prmSetCodeList.size();
        int resCode;
        double resEne;
        double resStrFct[lat.numCell];
        MPI_Status stts;
        ofstream rout;
        rout.open(outLoc+"strFctVrfy.dat");

        /*publish initial tasks to all*/
        for(int thrInd=1;thrInd<comm_size;thrInd++){
            prmSetCode=prmSetCodeList[prmSetCnt];
        ...
        while(prmSetCnt<(numPrmSet+comm_size-1)){
            ...
            if( (prmSetCnt+1)%100==0 ){
                iout<<(prmSetCnt+1)<<" parameters set calculated"<<endl;
                cout<<(prmSetCnt+1)<<" out of "<<prmSetCodeList.size()<<" parameters set calculated"<<endl;
            }
